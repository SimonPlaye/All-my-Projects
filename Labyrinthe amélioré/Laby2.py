"Convertisseur de labyrinthe en tkinter"
import os
from tkinter import *
import os
from tkinter import *
...
labyrinthe= cartes[nom_carte].split('\n')

L=[]
for i, elt in enumerate(labyrinthe):
    L.append([])
...
fen=Tk()
can=Canvas(fen, width=a, height=b, bg='#FFFFFF')
...
def deplacement(event):
    ...
    global rob_x1, rob_x2, rob_y1, rob_y2
    touche = event.char
    if touche == 'z' and rob_y1>longueur_case:
        rob_y1 -= longueur_case
        rob_y2 -= longueur_case

    if touche == 's' and rob_y2<b-longueur_case:
        rob_y1 += longueur_case
        rob_y2 += longueur_case

    if touche == 'q' and rob_x1>longueur_case:
        rob_x1 -= longueur_case
        rob_x2 -= longueur_case

    if touche == 'd' and rob_x2<a-longueur_case:
        rob_x1 += longueur_case
        rob_x2 += longueur_case
...
#Comprendre: pourquoi on ne peut pas supprimer le robot et le dessiner ailleurs
#c'est quoi une variable global
